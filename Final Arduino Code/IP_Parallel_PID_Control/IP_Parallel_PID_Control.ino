...
//Angle Calibration Without Voltage Divider:
float IP_UP = 789;
float IP_DOWN = 250;
...
/*
//Angle Calibration With Voltage Divider:
float IP_UP = 741;// /742
float IP_DOWN = 234;
float DEGREES_PER_READING = 2.78;
float MAX_READING = 1004;
*/
...
void setup() {
  ...
  */
  ...
  delay(200);
  findCenter();  //Locate the center of the track
  delay(200);
  TARGET_POSITION = newCenter;  //Set the target position to the center of the track
  Serial.println(currentPosition);
  Serial.println(newCenter);

  //Enable control timer:
  delay(200);                            //Delay to wait for initialization to complete
  MsTimer2::set(5, parallelPIDControl);  //Use Timer2 to set up 5ms timer interrupt for control loop
  MsTimer2::start();                     //Interrupt enable
}

void loop() {
  //Print to PuTTY for Plotting:
  time = millis();
  Serial.print(time - 5000);  //Time since system was restart - 5 seconds for initial set up
  Serial.print(",");
  Serial.print(theta - 19);  //Angle in degrees + adjustment for uneven potentiomter reading due to location and size of dead zone
  Serial.print(",");
...
float anglePIDController(float sensorReading) {
  ...
  anglePID = (angleKp * pError) + (iError * angleKi) + (dError * angleKd);  //Compute PID output
